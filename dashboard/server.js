...
const db=new Database(path.resolve(process.cwd(),'masterhosting.sqlite'),{readonly:true,fileMustExist:false});
...
function log(action,user,details){try{db.prepare('INSERT INTO logs(action,user,details) VALUES(?,?,?)').run(action,user,details||null)}catch{}}
...
app.get('/api/dashboard/members',async(req,res)=>{
  const r=await discord.get(`/guilds/${GUILD_ID}/members?limit=1000`);const members=r.data.map(m=>({id:m.user.id,tag:m.user.discriminator&&m.user.discriminator!=='0'?`${m.user.username}#${m.user.discriminator}`:m.user.username,roles:m.roles?.length||0}));res.json({members});
});
...
async function discordAction(req,res,type){
  const id=String(req.params.id||'').replace(/\D/g,'');if(!/^\d{15,25}$/.test(id))return res.status(400).json({error:'ID Discord invalide'});
  const reason=String(req.body?.reason||'Action effectuée depuis MasterHosting').slice(0,512);
  try{if(type==='ban')await discord.put(`/guilds/${GUILD_ID}/bans/${id}`,{delete_message_seconds:0,reason});if(type==='kick')await discord.delete(`/guilds/${GUILD_ID}/members/${id}`,{headers:{'X-Audit-Log-Reason':encodeURIComponent(reason)}});if(type==='timeout')await discord.patch(`/guilds/${GUILD_ID}/members/${id}`,{communication_disabled_until:new Date(Date.now()+Math.min(Math.max(Number(req.body.minutes)||5,1),40320)*60000).toISOString()},{headers:{'X-Audit-Log-Reason':encodeURIComponent(reason)}});log(type.toUpperCase(),actor(req),`target:${id} reason:${reason}`);res.json({ok:true})}catch(e){res.status(e.response?.status===404?404:502).json({error:'Discord a refusé l’action ou le membre est introuvable.'})}
}
